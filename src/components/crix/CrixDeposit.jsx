    ...
    <div className="p-6 bg-gray-900 rounded-xl border border-gray-800">
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {quickAmounts.map((amt) => (
          <button key={amt} onClick={() => setAmount(String(amt))} className="py-2 px-3 bg-gray-800 hover:bg-gray-700 text-white text-xs font-semibold rounded-lg transition">N{amt.toLocaleString()}</button>
        ))}
      </div>
      {breakdown && (
      ...
      <button onClick={handleDeposit} disabled={loading || !amount} className="w-full py-3 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-700 disabled:text-gray-500 text-white text-sm font-bold rounded-lg transition">
        {loading ? 'Opening checkout...' : 'Continue to Payment'}
      </button>
      <p className="mt-3 text-[10px] text-gray-500 text-center">Secured by CRIX - Cards, bank transfer, USSD supported</p>
    </div>
  );
}
